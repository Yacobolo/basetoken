 ...
 * <color-scheme-viewer>
 *
 * Visual configurator for the BaseToken CLI.
...
export class ColorSchemeViewer extends LitElement {
  ...
  private _buildCommand(): string {
    const parts = [`npx basetoken -s "${this.seed}"`];
  ...
  private _renderCommand() {
    ...
    return html`<span class="cmd-bin">npx</span> <span class="cmd-value">basetoken</span>${flag("-s", `"${this.seed}"`)}${this._format !== "oklch" ? flag("-f", this._format) : nothing}${this._variant !== "tonal-spot" ? flag("--scheme", this._variant) : nothing}${this._contrastLevel === 0.5 ? flag("--contrast", "medium") : this._contrastLevel === 1.0 ? flag("--contrast", "high") : nothing}${this._outputDir !== "./tokens" ? flag("-o", this._outputDir) : nothing}`;
  }
